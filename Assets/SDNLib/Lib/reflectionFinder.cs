...
public class reflectionFinder : MonoBehaviour
{
	...
	private static int targetNumReflections = 35;
	...
	void Start ()
	{
		boundary = GameObject.FindObjectOfType<boundary>();
...
//					Debug.Log("Update reflections");
	...
	private void updateDirectSound ()
	{
        ...
        layerMask = ~layerMask;
        if (Physics.Raycast(directSound.segments [0], out hit, Mathf.Infinity, layerMask)) {
	...
	private IEnumerator updateReflections ()
	{
		...
		for(int i = 0; i < initalDirections.Length; i++) {
			...
			while (!currPath.isValid && numTry < targetNumReflections) {   // CHANGED HERE FROM 8 TO TARGET NUM
			...
			if (currPath.isValid) {
                ...
                //Debug.Log(currPath.wallNum + "  1 " + currPath.segments[1].origin);
            }
            else {
				...
				if (currPath.isValid) {
					reflections.Add (currPath);
                    //Debug.Log(currPath.wallNum + "  2  " + currPath.segments[1].origin);
                }
				boundary.turnOffCollider ();
			}
	...
	private bool validatePath (reflectionPath p, string coll)
	{
        ...
        if (!Physics.Raycast(p.segments[0], out incomingHit, Mathf.Infinity, layerMask))
        {
            //Debug.Log("1 " + coll);
            //return false;
        }
        ...
        for (int i = 0; i < reflections.Count; i++)
        {
            ...
            if (Vector3.Distance(p.segments[1].origin, cp.segments[1].origin) < 0.01f)
            {
                //Debug.Log("3 " + coll);
                return false;
            }
        ...
        if (!Physics.Raycast(p.segments[1], out outgoingHit, Mathf.Infinity, layerMask))
        {
            //Debug.Log("4 " + coll);
            return false;
        }
    ...
    private int getWallNum(string colliderName) // not used
    {
        // 1=right, 2=front, 3=left, 4=back, 5=ceiling, 6=floor
        ...
        switch (colliderName)
        {
            ...
            case "floor":
                return 6;
            default:
                Debug.Log("Oops ! No matching walls.");
                return 0;
        }
    }
