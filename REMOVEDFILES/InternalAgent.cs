...
public class InternalAgent : Agent {
    ...
    float learning_rate = 0.2f; // The rate at which to update the value estimates given a reward.
    int action = -1;
    float gamma = 0.15f; // Discount factor for calculating Q-target.
    float e = 0.7f; // Initial epsilon value for random action selection.
    float eMin = 0.1f; // Lower bound of epsilon.
    int annealingSteps = 100; // Number of steps to lower e to eMin.
    int lastState;

    //CREA LO SPAZIO PARAMETRI E LO INZIALIZZA A 0
...
//        Debug.Log("Step di Q-Learning: ");
        ...
        if (Random.Range(0f, 1f) < e) {
            ...
            //MODIFICARE IN BASE AL NUMERO DI AZIONI!!!!!!
